...
import { Download, FileText, Search, Plus, Mail, CheckCircle, AlertCircle, Clock, FileEdit, Stamp, Trash2, DollarSign, Info, Settings, RefreshCw, Store, Calendar, List, Camera, Sparkles, Upload } from 'lucide-react';
...
function InvoicingContent() {
    const { tier, folios } = useSubscription();
    ...
    const [emailModal, setEmailModal] = useState<{ isOpen: boolean, invoice: any, email: string } | null>(null);
    ...
    // [NEW] Callback for actions inside preview (Print, Email, etc)
    const [previewActionHandler, setPreviewActionHandler] = useState<((action: string, inv: any) => void) | null>(null);
    const [successModalData, setSuccessModalData] = useState<any>(null);

    const handlePreview = (invoice: any, actionHandler?: (action: string, inv: any) => void) => {
        // Fetch fresh details if needed (or pass flattened invoice)
        setPreviewInvoice(invoice);
        if (actionHandler) {
            setPreviewActionHandler(() => actionHandler);
        } else {
            setPreviewActionHandler(null);
        }
    };
    ...
    const loadDocs = async () => {
        try {
            ...
            const dbDrafts = txs
                .filter((t: any) => t.category?.includes('Pre-CFDI') || (t.type === 'income' && t.has_invoice === true && !t.details?.uuid))
                .map((t: any) => ({
                    id: t.id,
                    folio: 'PRE-' + t.id,
            ...
            const dbInvoices = txs
                .filter((t: any) => !!t.has_invoice && t.category !== 'Factura Cancelada / Oculto' && t.details?.status)
                .map((t: any) => {
                    return {
                        id: t.id,
                        folio: (() => {
                            const raw = t.invoice_number || t.details?.folio || `F-${new Date(t.date).getFullYear()}${t.id.toString().slice(-3)}`;
                            const ser = t.details?.series;
                            return (ser && !raw.startsWith(ser)) ? `${ser}${raw}` : raw;
                        })(),
                        date: formatSafeDate(t.details?.date || t.date),
                        rawDate: t.details?.date || (t.date.includes('T') ? t.date : `${t.date}T12:00:00`),
                        client: t.details?.client || t.description.split(' - ')[0] || 'Cliente',
                        rfc: t.details?.rfc || '',
                        total: t.amount,
                        status: t.details?.status || 'unknown',
                        cancellationDate: t.details?.cancellation_date || t.details?.cancellationDate,
                        sent: t.details?.sent || false,
                        details: {
                            ...t.details,
                        ...
                        paymentReceived: t.payment_received, // [FIX V68] Source of Truth
                        // Helper flag
    ...
    useEffect(() => {
        if (activeTab === 'drafts' || activeTab === 'issued' || activeTab === 'cancelled') {
            ...
            // [SYNC] Fetch remote invoice changes
            supabaseService.syncInvoicesFromFacturapi().then(() => {
                console.log("SYNC: UI Refreshing after sync...");
                loadDocs();
            });
        ...
        try {
            ...
            const dbInvoices = updatedTxs
                ...
                .map((t: any) => ({
                    ...
                    rfc: t.details?.rfc || '',
                    total: t.amount,
                    status: (t.details?.status === 'cancelled' || t.details?.status === 'canceled') ? 'cancelled' : (t.payment_received ? 'paid' : 'pending'),
                    uuid: t.details?.uuid || '',
                    sent: false,
                    xml: t.details?.xml || '',
                    details: t.details || {},
    ...
    const handleModalSave = async (data: any) => {
        try {
            ...
            const realInvoices = updatedTxs
                .filter((t: any) => !!t.has_invoice && t.category !== 'Factura Cancelada / Oculto')
    ...
    // [NEW] Handle Duplicate (Clone Invoice)
    const handleDuplicate = (invoice: any) => {
        // Clone and strip sensitive data
        const clonedData = {
            ...invoice.details, // Use details as source of truth
            client: invoice.client || invoice.details?.client,
            rfc: invoice.rfc || invoice.details?.rfc,
            // Fiscal Data
            fiscalRegime: invoice.details?.fiscalRegime || '626',
            cfdiUse: invoice.details?.cfdiUse || 'G03',
            zip: invoice.details?.zip || '',
            address: invoice.details?.address || '',
            email: invoice.details?.email || '', // Preserve Email

            // Concepts (Single concept support for now as per UI)
            description: invoice.description || invoice.details?.description,
            satProductKey: invoice.details?.satProductKey || '85121610',
            satUnitKey: invoice.details?.satUnitKey || 'E48',
            quantity: invoice.details?.quantity || 1,
            unitValue: invoice.details?.unitValue || invoice.total,
            subtotal: invoice.details?.subtotal || invoice.total,
            iva: invoice.details?.iva || 0,
            hasIva: (invoice.details?.iva > 0),
            retention: invoice.details?.retention || 0,
            hasRetention: (invoice.details?.retention > 0),

            // Payment
            paymentMethod: invoice.details?.paymentMethod || 'PUE',
            paymentForm: invoice.details?.paymentForm || '03',

            // STRIP Identifiers
            id: undefined,
            uuid: undefined,
            folio: undefined, // Let system generate new Draft Folio
            date: undefined, // Will default to Today
            status: 'draft',
            ...
            satCertificateNumber: undefined
            // keep 'xml' undefined
        };
        ...
        toast('Borrador creado desde duplicado', { icon: '📋' });
    };
    ...
    const sendEmail = async (invoice: any) => {
        console.log("📧 Opening Email Modal for:", invoice?.id);
        try {
            // [Fix] Lookup Email if missing (JIT)
            let defaultEmail = invoice?.details?.email || invoice?.details?.customer?.email || '';

            if (!defaultEmail && invoice?.details?.rfc) {
                // [Fix] 1. Try to find email from existing transactions/clients by RFC
                const allTxs = await supabaseService.getTransactions();
                // Find latest transaction with same RFC that has an email
                const match = allTxs.find((t: any) =>
                    t.details?.rfc === invoice.details.rfc && (t.details?.email || t.details?.customer?.email)
                );
                if (match) {
                    defaultEmail = match.details?.email || match.details?.customer?.email;
                }

                // [Fix] 2. If still empty, fetch from Fiscal Clients API (Source of Truth)
                if (!defaultEmail) {
                    try {
                        const clientRes = await fetch(`/api/sat/clients?q=${invoice.details.rfc}`);
                        if (clientRes.ok) {
                            const clientData = await clientRes.json();
                            // API returns { data: [...] }
                            const foundClient = clientData.data?.find((c: any) => c.tax_id === invoice.details.rfc);
                            if (foundClient?.email) {
                                defaultEmail = foundClient.email;
                                console.log("Shape found from API:", defaultEmail);
                            }
                        }
                    } catch (err) {
            ...
            toast.error("Error al abrir modal: " + e.message);
        }
    };
    ...
    const processEmailSend = async () => {
        ...
        setEmailModal(null); // Close modal

        const toastId = toast.loading("Generando PDF Premium...");
        try {
            // [NEW] Get Current User for Reply-To & CC
            const user = await supabaseService.getCurrentUser();
            const replyTo = user?.email || '';
            const senderName = user?.user_metadata?.full_name || user?.user_metadata?.name || 'Dr. Usuario';
            const ccEmail = user?.email || ''; // CC the sender

            // Prepare robust data
            const pdfData = await preparePdfData(invoice);
            const blob = await pdf(<InvoiceDocument data={pdfData} />).toBlob();

            // Convert to Base64 (Promisified)
            const base64data = await new Promise<string>((resolve, reject) => {
                ...
                reader.onloadend = () => {
                    ...
                    else reject(new Error("Error al convertir PDF a Base64"));
                };
                reader.onerror = () => reject(new Error("Error de lectura de archivo"));
            });

            // [NEW] Prepare XML Base64 if available
            let xmlBase64 = null;
            console.log("SEND EMAIL DEBUG: Invoice Object:", invoice);
            console.log("SEND EMAIL DEBUG: XML Available?", !!invoice.xml);

            if (invoice.xml) {
                try {
                    // Safe UTF-8 to Base64 conversion
                    xmlBase64 = window.btoa(unescape(encodeURIComponent(invoice.xml)));
                    console.log("SEND EMAIL DEBUG: XML Encoded successfully, length:", xmlBase64.length);
                } catch (e) {
                    console.error("Error encoding XML", e);
                }
            } else {
                console.warn("SEND EMAIL DEBUG: No XML found in invoice object.");
            }
            ...
            const res = await fetch('/api/sat/email', {
                ...
                body: JSON.stringify({
                    ...
                    facturapiId: invoice.details?.id, // [NEW] Explicit ID for JIT Recovery
                    email,
                    cc: ccEmail, // [NEW] Send copy to issuer
                    pdfBase64: base64data,
                    xmlBase64: xmlBase64, // [NEW] Send XML
                    filename: `Factura_${invoice.folio || 'CFDI'}.pdf`,
                    xmlFilename: `Factura_${invoice.folio || 'CFDI'}.xml`,
                    replyTo,
                    senderName
                })
            });
            ...
            toast.success(`✅ Enviado al Cliente y copia a ${ccEmail}`, { id: toastId });
    ...
    return (
        <div className="space-y-6 pb-20 relative overflow-x-hidden">
            ...
            <InvoicePreview
                ...
                onAction={async (action) => {
                    if (action === 'download') {
                        ...
                        // [FIX] Handle Draft Deletion (No UUID) vs SAT Cancellation
                        const isDraft = !previewInvoice.uuid && !['paid', 'cancelled'].includes(previewInvoice.status);

                        const isConfirmed = await confirmAction({
                            title: isDraft ? "Eliminar Borrador" : "Cancelar Factura",
                            message: isDraft
                                ? "¿Estás seguro de eliminar este borrador permanentemente?"
                                : "⚠️ ¿Realmente deseas CANCELAR esta factura ante el SAT?\n\nEsta acción es irreversible y anulará el folio fiscal (UUID).",
                            confirmText: isDraft ? "Eliminar" : "Sí, Cancelar Factura",
                            cancelText: isDraft ? "Conservar" : "No cancelar",
                            variant: "danger"
                        });
                        ...
                        if (isConfirmed) {
                            try {
                                if (isDraft) {
                                    // 1. Delete Draft
                                    await supabaseService.deleteTransaction(previewInvoice.id);
                                    setDrafts(prev => prev.filter(d => d.id !== previewInvoice.id));
                                    toast.success('Borrador eliminado.');
                                } else {
                                    // 2. Cancel in Facturapi
                                    const satService = await import('@/services/satService').then(m => m.satService);
                        ...
                        // [FIX] Pass logoUrl explicitly to ensure valid PDF generation with fallback
                        await sendEmail({ ...previewInvoice, logoUrl: organizationLogo || previewInvoice.logoUrl || "/logo-synaptica.png" });
                    }
                }}
            />
                    ...
                    <div className="flex items-center gap-2 w-full">
                        ...
                        <button
                            onClick={async () => {
                                ...
                                try {
                                    const res: any = await supabaseService.syncInvoicesFromFacturapi();
                                    const count = Array.isArray(res) ? res.length : (res.invoices?.length || 0);

                                    toast.dismiss(toastId);
                                    if (res.debug?.error) {
                                        toast.error("Error sincronizando: " + res.debug.message);
                                    } else {
                                        toast.success(`Sincronización Completada: ${count} facturas.`);
                                        setRefreshTrigger(prev => prev + 1);
                                    }
                                } catch (e: any) {
                                    toast.dismiss(toastId);
                            ...
                            className="p-2 text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition"
                            title="Sincronizar Facturas"
                        >
                            <RefreshCw size={20} />
                        </button>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-6">
                                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                                    ...
                                    {(() => {
                                        const remaining = folios; // Use Real Data from Context
                                        ...
                                        return (
                                            <div className={`border rounded-lg p-4 ${remaining < 5 ? 'bg-red-50 border-red-200' : 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800'}`}>
                                                <p className={`text-sm ${remaining < 5 ? 'text-red-800' : 'text-green-800 dark:text-green-300'}`}>
                                                    Tienes <strong>{remaining} folios</strong> disponibles.
                                                    <br /><span className="text-xs opacity-75">Sincronizado con SAT</span>
                                                </p>
                                            </div>
                                        );
                                    })()}
                                    <Link
                                        ...
                                        className="w-full mt-4 py-2 border border-indigo-200 text-indigo-700 hover:bg-indigo-50 rounded-lg text-sm font-semibold transition block text-center"
                                    >
                                        Comprar más folios
                                    </Link>
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        onClick={async () => {
                                            const toastId = toast.loading("Sincronizando...");
                                            try {
                                                const res = await supabaseService.syncInvoicesFromFacturapi();
                                                await loadDocs(); // Reload UI
                                                toast.dismiss(toastId);
                                                toast.success(`Sincronización completa. Invoices: ${res.invoices.length}`);
                                            } catch (e: any) {
                                                toast.dismiss(toastId);
                                                toast.error("Error: " + e.message);
                                            }
                                        }}
                                        className="p-1.5 ml-2 text-sky-600 hover:bg-sky-100 rounded-full transition"
                                        title="Sincronizar y Restaurar"
                                    >
                                        <RefreshCw size={16} />
                                    </button>
                                </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700 text-sm">
                                        {invoices
                                            ...
                                            .map((inv) => (
                                                <tr key={inv.id}
                                                    onClick={() => handlePreview(inv, async (action, previewInvoice) => {
                                                        if (action === 'print') handlePremiumPrint(previewInvoice);
                                                        if (action === 'email') sendEmail(previewInvoice);
                                                        if (action === 'duplicate') handleDuplicate(previewInvoice); // [NEW] Wired
                                                        if (action === 'download') handlePremiumDownload(previewInvoice);
                                                        if (action === 'cancel') {
                                                            const isConfirmed = await confirmAction({
                                                ...
                                                >
                                                    ...
                                                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                                                        <div className="flex items-center justify-center gap-2">
                                                            {(inv.status === 'paid' || inv.paymentReceived) && <span title="Timbrado / Vigente (Pagado)"><CheckCircle size={18} className="text-green-500" /></span>}
                                                            {(!inv.paymentReceived && (inv.status === 'pending' || inv.status === 'valid')) && <span title="Timbrado / Vigente"><CheckCircle size={18} className="text-green-500" /></span>}
                                                            ...
                                                            <button
                                                                ...
                                                                disabled={inv.status === 'paid' || inv.status === 'cancelled' || inv.paymentReceived}
                                                                className={`transition p-1 rounded-full ${(inv.status === 'paid' || inv.status === 'cancelled' || inv.paymentReceived) ? 'cursor-default opacity-100' : 'cursor-pointer hover:bg-green-50 hover:text-green-600'}`}
                                                                title={(inv.status === 'paid' || inv.paymentReceived) ? "Pagado" : "Clic para Registrar Cobro"}
                                                            >
                                                                <DollarSign size={18} className={(inv.status === 'paid' || inv.paymentReceived) ? "text-green-600" : "text-slate-300 dark:text-slate-600"} />
                                                            </button>
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center justify-center gap-3">
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); handleStamp(draft.id); }}
            ...
            {/* Client-Side Invoice Preview Portal */}
            {previewInvoice && (
                <InvoicePreview
                    isOpen={!!previewInvoice}
                    onClose={() => setPreviewInvoice(null)}
                    data={previewInvoice}
                    onAction={(action) => {
                        // Priority: Custom Handler passed via handlePreview, else default logic
                        if (previewActionHandler) {
                            previewActionHandler(action, previewInvoice);
                        } else {
                            // Legacy / Default Fallback for standalone preview
                            if (action === 'duplicate') handleDuplicate(previewInvoice); // Direct wire if no handler passed
                        }
                    }}
                />
            )}
            ...
            {/* Email Prompt Modal (Replaces window.prompt) */}
            {emailModal?.isOpen && typeof document !== 'undefined' && createPortal(
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden border border-slate-200 dark:border-slate-700">
                        <div className="p-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="font-bold text-lg text-slate-800 dark:text-white">Enviar Factura por Correo</h3>
                            </div>

                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                                Ingresa la dirección de correo electrónico del destinatario. <br />
                                <span className="text-xs text-indigo-500 font-medium">Se enviará una copia automática a tu correo.</span>
                            </p>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all mb-6 text-sm"
                                autoFocus
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') processEmailSend();
                                    if (e.key === 'Escape') setEmailModal(null);
                                }}
                            />
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition"
                                >
                                    Cancelar
                                </button>
                                <button
                                    onClick={processEmailSend}
                                    disabled={!emailModal.email}
                                    className="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-500/20 transition disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Enviar
                                </button>
                            </div>
